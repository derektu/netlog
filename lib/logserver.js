...
var LogServer = function(options, logDB) {
    ...
    // uri: /api/<appID>/<logID>/loglist?seqno=..&count=..
    //  return list of logs of appID/logID of the specified range
    //  - seqno: optional, 如果不傳 or 0, 表示從最新的資料讀起
    //  - count: optional, 欲讀取的資料筆數
    //
    //  回傳的資料是依照log寫入順序, i.e. 舊的資料在前面
    //
    //  support JSONP ('callback=..')
    //
    this.expressServer.get('/api/:appID/:logID/loglist', function(req, res) {
        try {
            var seqno = req.query.seqno || 0;
            var count = req.query.count || 100;

            logDB.getLogData(req.params.appID, req.params.logID, seqno, count,
                function(logData) {
                    res.jsonp(logData);
                },
                function(err) {
